...
# export and contain no valid trial data. The guard below ensures the removal
# is only applied to the expected 471-row export (464 valid trials remain).
stopifnot(nrow(df) == 471)
df <- df[-c(465:471), ]
stopifnot(nrow(df) == 464)
...
boxM_result <- suppressWarnings(boxM(df[, main_vars], df$group))
print(boxM_result)
...
# NOTE: these models are fitted with brms default priors. The explicit
# prior specification in Section 11 is used for prior predictive
# simulation only (sample_prior = "only") and is NOT applied here.
